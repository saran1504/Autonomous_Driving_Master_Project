...
float sensor_model::SignProbability(const std::vector<Sign>& observed_signs_map, Eigen::Transform<float,3,Eigen::Affine> particle_transform){
    float prob = 0.0f;
    float norm_val = 0;
    // Observed Signs
...
float sensor_model::LaneProbability(const std::vector<Eigen::Vector3f>& observed_points, const std::vector<Eigen::Vector3f>& data_associations, const std::vector<float>& weights, const float total_weight){
...
//    bool success = false; // Dummy for compilation
    ...
    // const std::vector<Sign> world_signs = transformSigns(observed_signs, ANY_PARTICLE.transform);
    if (use_lane_reg_) {
...
// const std::vector<Sign> world_signs = transformSigns(observed_signs, particles[0].transform);
        //std::cout<<"REG MATRIX"<<lane_regression.at(0)<<std::endl;
        weights.clear();
        ...
        //std::cout<<"PRINNNNNNNNT"<<lane_regression.at(0).size << std::endl;
        for (int i = 0; i < 600; i++) {
            for (int j = 0; j < 600; j++) {
                std::cout<<lane_regression.at(0).at<float>(i,j)<<std::endl;
                if (lane_regression.at(0).at<float>(i, j) < 200) {
        ...
        for (int i_p = 0; i_p < particles.size(); i_p++) {
            ...
            float prob_sign = SignProbability(world_signs, particles[i_p].transform);
            ...
            float prob_lane = LaneProbability(points_trans, corr_center_points, reg_weights, tot_prob);
//            if (abs(prob_lane + prob_sign) <= prob_sign) {
//                particles[i_p].weight = prob_sign;
//                temp_weight += prob_sign;
//                weights.push_back(prob_sign);
//                std::cout << "Using only signs" << std::endl;
//            } else {
                particles[i_p].weight = (prob_sign + prob_lane) / 2;
                temp_weight += (prob_sign + prob_lane) / 2;
                weights.push_back((prob_sign + prob_lane) / 2);
                std::cout << "Using Both" << std::endl;
//            }
            if (prob_sign > max_prob) {
                max_prob = prob_sign;
