...
public:
    ...
    float SignProbability(const std::vector<Sign>& observed_signs, Eigen::Transform<float,3,Eigen::Affine> particle_transform);
    std::vector<Sign> transformSigns(const std::vector<Sign>& signs, const Eigen::Transform<float,3,Eigen::Affine>& particle_pose);
    float LaneProbability(const std::vector<Eigen::Vector3f>& observed_points, const std::vector<Eigen::Vector3f>& data_associations, const std::vector<float>& weights, const float total_weight);
