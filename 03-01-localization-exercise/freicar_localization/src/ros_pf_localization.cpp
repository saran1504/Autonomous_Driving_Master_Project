...
Localizer::Localizer(std::shared_ptr<ros::NodeHandle> n) : n_(n), it_(*n) {
    ...
    odo_sub_ = n_->subscribe(car_name+"/odometry", 1, &Localizer::OdoCallback, this);
    marker_sub_ = n_->subscribe(car_name+"/traffic_signs", 1, &Localizer::markerCallback, this);
    ...
    if (use_lane_reg_) {
        image_sub_ = it_.subscribe(car_name+"/sim/camera/rgb/front/reg_bev", 1, &Localizer::RegCallback, this);
    }
...
void Localizer::OdoCallback(const nav_msgs::OdometryConstPtr &msg) {
    ...
    stf.child_frame_id = car_name + "/handle";
...
void Localizer::StartObservationStep(){
    ...
    if (use_lane_reg_) {
        if (!latest_lane_reg_.first.empty()) {
            //std::cout<<latest_lane_reg_.second << "          " << last_odo_update_<<std::endl;
            if (abs((latest_lane_reg_.second - last_odo_update_).toSec()) > 0.0001) {
                std::cout<<"ENTERED SM IF"<< latest_lane_reg_.first.at(0).size<<std::endl;
    ...
    //std::cout<< "LAUDA KA BAAL"<< observation_reg.at(0).size << std::endl;
...
void Localizer::RegCallback(const sensor_msgs::ImageConstPtr &msg) {
    ...
    std::cout<<"REG CALLBACK MAT"<< mat_vec.at(0).size <<std::endl;
