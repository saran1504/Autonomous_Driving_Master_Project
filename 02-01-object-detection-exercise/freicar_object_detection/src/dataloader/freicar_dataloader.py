...
########################################################################
def data_augmentation(img, aug_prob):
    '''
    ...
    if aug_val <= aug_prob:
        if mode == 0 or mode == 2:
            ...
            if br_dr >= 0.5:
                hsvImg = cv2.cvtColor(img_temp, cv2.COLOR_BGR2HSV)
                value = np.random.randint(10, 30)
                vValue = hsvImg[..., 2]
                img_temp[..., 2] = np.where((255 - vValue) < value, 255, vValue + value)

            else:
                factor = np.random.uniform(0.5, 0.9)
                hsvImg = cv2.cvtColor(img_temp, cv2.COLOR_BGR2HSV)
                img_temp[..., 2] = hsvImg[..., 2] * factor
        ...
        if mode == 1 or mode == 2:
            ...
            if crop_prob >= 0.5:
                ...
                img_temp = img[y:y + height, x:x + width]
                ...
                img_temp = cv2.blur(img_temp, (x_blur, y_blur))
...
def loadRGB(paths, size=None, pad_info=None):
    ...
    rgb_im = data_augmentation(rgb_im[0], 0.5)  # please comment this line if you do not want to use data augmentation
    if size is not None:
...
class FreiCarDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in info:
            for bbox_val in info[i]:
                bbox_val['y'] = bbox_val['y'] + 12
