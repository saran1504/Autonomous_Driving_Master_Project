#include "freicar_sign_detect_node.h"

using cv::Ptr;
using cv::Vec3d;
using cv::Point2f;

FreicarSignDetect::FreicarSignDetect(const std::unique_ptr<ros::NodeHandle> &nodehandle, const std::string agent_name): image_transport_(*nodehandle)
{
    agent_name_ = agent_name;
    image_sub_  = std::make_unique<image_transport::Subscriber>(image_transport_.subscribe(agent_name + "/sim/camera/rgb/front/image", 1, &FreicarSignDetect::ImageCallback, this));
    ar_detect_  = std::make_unique<ArucoDetect>(nodehandle, agent_name);
...
// ARUCO !!!!!!!!!!!!!!!!!!!!!!
ArucoDetect::ArucoDetect(const std::unique_ptr<ros::NodeHandle> &nodehandle, std::string name):cam_initialized_(false)
{
...
void ArucoDetect::publishRos(std::vector<Vec3d> & Tvec, std::vector<Vec3d> & Rvec, std::vector< int > & ids, ros::Time stamp)
{
...
int main(int argc, char **argv)
{
    ...
    std::unique_ptr<ros::NodeHandle> n = std::make_unique<ros::NodeHandle>();

    std::string agent_name;
    if (!ros::param::get("/sign_detector/agent_name", agent_name)) {
